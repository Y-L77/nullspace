  ...
  if (grid) {
    ...
    for (let x = sx; x < camX + w / zoom + DOT; x += DOT) {
      for (let y = sy; y < camY + h / zoom + DOT; y += DOT) {
        ctx.beginPath()
        ctx.arc((x - camX) * zoom, (y - camY) * zoom, Math.max(0.7, 0.9 * zoom), 0, Math.PI * 2)
        ctx.fill()
      }
    }
  }
...
export default function Canvas() {
  ...
  const moveSnap = useRef<ReturnType<typeof canvas.snapshot> | null>(null)
  const [selected, setSelected] = useState<string | null>(null)
  const drag = useRef<{ dx: number; dy: number } | null>(null)
  const blockDrag = useRef<{ id: string; dx: number; dy: number } | null>(null)
  const [draggingBlock, setDraggingBlock] = useState(false)
  const [trash, setTrash] = useState(false)

  const [latexBox, setLatexBox] = useState<Overlay | null>(null)
  const [latexVal, setLatexVal] = useState('')
  const latexRef = useRef<HTMLInputElement>(null)
  const [textBox, setTextBox] = useState<Overlay | null>(null)
  ...
  const redraw = useCallback(() => {
    ...
    if (thumb) {
      const visibleH = el.height / zoomRef.current
      thumb.style.height = `${Math.max(40, (visibleH / height) * el.height)}px`
      thumb.style.top = `${(camY.current / height) * el.height}px`
    }
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      ...
      setCam(camX.current, camY.current)
      redraw()
    })
  ...
  const exportPdf = useCallback(() => {
    const el = canvasRef.current
    if (!el) return
    const pageW = Math.max(el.width, PDF_W)
    const pageH = PDF_H
    const pageContent = pageH - PDF_M * 2
    const total = Math.max(1, Math.ceil(height / pageContent))
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (!ctx) return
      const top = i * pageContent - PDF_M
      draw(ctx, canvas.strokes, 0, top, pageW, pageH, grid, 1)
      for (const b of blocks) {
        const y = b.y - top
        if (y < -120 || y > pageH + 120) continue
        drawText(ctx, b.kind === 'text' ? b.source : `$${b.source}$`, b.x + 7, y + 4, b.kind === 'text' ? '15px system-ui, sans-serif' : '16px serif')
      }
      ctx.fillStyle = '#666'
      ctx.font = '11px system-ui'
      ctx.fillText(`${activeTab?.name ?? 'Nullspace'} · page ${i + 1}/${total}`, PDF_M, pageH - 22)
  ...
  useEffect(() => {
    const key = (e: KeyboardEvent) => {
      ...
      if ((e.key === 'Delete' || e.key === 'Backspace') && selected) {
        ...
        const s = canvas.strokes.find(x => x.id === selected)
        if (b) canvas.removeLatexBlock(selected)
        else if (s) canvas.removeStroke(selected, canvas.snapshot())
        setSelected(null)
      }
  ...
  useEffect(() => {
    ...
    const zoomTo = (e: Event) => {
      ...
      const centerY = camY.current + (el?.height ?? 800) / (2 * old)
      zoomRef.current = z
      setZoomState(z)
    ...
    return () => {
      ...
      window.removeEventListener('nullspace:zoom', zoomTo)
    }
  ...
  const onPointerDown = (e: React.PointerEvent) => {
    ...
    if (tool === 'latex' || tool === 'text') {
      const r = canvasRef.current!.getBoundingClientRect()
      const box = { x: e.clientX - r.left, y: e.clientY - r.top, camX: camX.current, camY: camY.current }
      if (tool === 'latex') {
        setLatexBox(box)
        setLatexVal('')
        setTimeout(() => latexRef.current?.focus(), 10)
      } else {
        setTextBox(box)
        setTextVal('')
    ...
    if (tool === 'cursor') {
      const p = world(e.clientX, e.clientY)
      const hit = [...canvas.strokes].reverse().find(s => hitStroke(s, p.x, p.y))
      if (hit) {
        setSelected(hit.id)
        moveSnap.current = canvas.snapshot()
        const cx = hit.points.reduce((a, p) => a + p.x, 0) / hit.points.length
        const cy = hit.points.reduce((a, p) => a + p.y, 0) / hit.points.length
        drag.current = { dx: p.x - cx, dy: p.y - cy }
        e.currentTarget.setPointerCapture(e.pointerId)
      } else {
        setSelected(null)
        drag.current = null
        moveSnap.current = null
      }
      return
    }
    ...
    e.currentTarget.setPointerCapture(e.pointerId)
    drawing.current = true
    snap.current = canvas.snapshot()
    const p = world(e.clientX, e.clientY)
    canvas.startStroke(p.x, p.y, color, tool === 'highlight' ? Math.max(lineWidth * 4, 16) : lineWidth, tool === 'highlight' ? 0.35 : 1)
  }
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    if (tool === 'cursor' && selected && drag.current) {
      const p = world(e.clientX, e.clientY)
      const s = canvas.strokes.find(s => s.id === selected)
      if (!s) return
      const cx = s.points.reduce((a, p) => a + p.x, 0) / s.points.length
      const cy = s.points.reduce((a, p) => a + p.y, 0) / s.points.length
      canvas.moveStroke(selected, (p.x - drag.current.dx) - cx, (p.y - drag.current.dy) - cy)
      return
    }
  ...
  const onPointerUp = () => {
    ...
    if (tool === 'eraser') { drawing.current = false; return }
    if (!drawing.current) return
    drawing.current = false
    canvas.endStroke(snap.current!)
    snap.current = null
  }
  ...
  const editBlock = (b: Block) => {
    const sx = (b.x - camX.current) * zoomRef.current
    const sy = (b.y - camY.current) * zoomRef.current
    if (b.kind === 'text') {
      setTextBox({ x: sx, y: sy, camX: camX.current, camY: camY.current, editingId: b.id })
      setTextVal(b.source)
      setTimeout(() => textRef.current?.focus(), 10)
    } else {
      setLatexBox({ x: sx, y: sy, camX: camX.current, camY: camY.current, editingId: b.id })
      setLatexVal(b.source)
      setTimeout(() => latexRef.current?.focus(), 10)
    }
  }
  ...
  const startBlockDrag = (e: React.PointerEvent<HTMLDivElement>, b: Block) => {
    ...
    moveSnap.current = canvas.snapshot()
    blockDrag.current = { id: b.id, dx: p.x - b.x, dy: p.y - b.y }
  ...
  return <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg)', minWidth: 0, maxWidth: '100%' }}>
    <div style={{ padding: '11px 20px', borderBottom: '1px solid var(--border)', fontSize: 13, color: 'var(--text2)', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 10, overflow: 'hidden' }}>
      ...
      <span style={{ fontSize: 10, color: 'var(--text3)' }}>[{tool}]</span>
    ...
    <div ref={containerRef} style={{ flex: 1, position: 'relative', overflow: 'hidden', minWidth: 0, maxWidth: '100%' }} onContextMenu={e => e.preventDefault()}>
      ...
      {blocks.map(b => {
        ...
        const sx = (b.x - camera.x) * zoom
        const sy = (b.y - camera.y) * zoom
        const style: CSSProperties = { position: 'absolute', left: sx, top: sy, transform: `scale(${zoom})`, transformOrigin: 'top left', color: '#e8e6e1', fontSize: 15, lineHeight: isText ? 1.45 : undefined, pointerEvents: tool === 'cursor' ? 'auto' : 'none', userSelect: 'none', cursor: tool === 'cursor' ? 'move' : 'default', background: isText ? 'rgba(20,20,20,0.08)' : 'transparent', borderRadius: 4, padding: isText ? '4px 7px' : '2px 6px', border: selected === b.id ? '1px solid #6fa3d4' : '1px solid transparent', whiteSpace: isText ? 'pre-wrap' : undefined, minWidth: isText ? 80 : undefined, maxWidth: isText ? 520 : undefined, fontFamily: isText ? 'system-ui, sans-serif' : undefined }
        return isText
          ? <div key={b.id} style={style} onPointerDown={e => startBlockDrag(e, b)} onPointerMove={moveBlockDrag} onPointerUp={stopBlockDrag} onPointerCancel={stopBlockDrag} onDoubleClick={() => tool === 'cursor' && editBlock(b)}>{b.source}</div>
